# Zyvor Argus — User Documentation
...
| Browse capabilities by theme | [Feature Guide](../zyvor-argus-user-feature-guide.md) |
...
| PDF | Contents |
|-----|----------|
| `ZyvorArgus-User-README.pdf` | This overview |
...
The build regenerates indexes first. Check links with `node scripts/user-docs/check-links.mjs`.

Publish to the product site:

```bash
node scripts/user-docs/sync-to-website.mjs ../zyvorai-web
```
...
Deep engineering docs live under [`../`](../) (architecture, tutorials, configuration) and are out of scope for this user package.
