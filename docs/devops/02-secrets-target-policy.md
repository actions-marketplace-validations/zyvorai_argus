...
| OIDC client secret / Keycloak admin | Argus Enterprise SSO | Helm stableSecret / vault — see [`docs/user/enterprise-sso.md`](../user/enterprise-sso.md) |
...
- Argus Enterprise demo Keycloak users (`demo`/`demo`, `ssouser`/`Sso@321`) are for smoke tests only — disable `keycloak.createTestUsers` outside eval labs ([Enterprise SSO](../user/enterprise-sso.md)).
